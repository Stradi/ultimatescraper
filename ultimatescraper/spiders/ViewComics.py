...
class ViewcomicsSpider(scrapy.Spider):
  ...
  def parse_comic(self, response):
    def parse_table():
      table_keys = response.css('.full-table tr td:nth-child(1) *::text').getall()
      table_values = response.css('.full-table tr td:nth-child(2) *::text').getall()
  ...
  def parse_issue(self, response, data):
    ...
    try:
      ...
      del data['_issues']
      logger.debug("Parsed {comicname} page, passing it to pipelines.".format(comicname=data['name']))
